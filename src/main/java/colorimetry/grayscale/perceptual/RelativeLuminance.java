...
public final class RelativeLuminance implements Grayscale {
    ...
    public double[] toGrayXyz(double[] xyz) {
        double[] rgb = ColorConverter.convert(Xyz.INSTANCE, Bt709RgbLinear.INSTANCE, xyz);
        double y = WR * rgb[0] + WG * rgb[1] + WB * rgb[2];
        
        return ColorConverter.convert(Bt709RgbLinear.INSTANCE, Xyz.INSTANCE, new double[]{y, y, y});
    }
}