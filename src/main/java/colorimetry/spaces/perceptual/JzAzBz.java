 ...
 * Quantizer (SMPTE ST 2084) transfer function internally with a re-optimized m2
 * exponent.
 * 
 * SDR white (Y=1 in the library's XYZ scale) is mapped to 203 cd/m² per
 * ITU-R BT.2408 before PQ encoding.
 */
public final class JzAzBz implements LabLike {
    public static final JzAzBz INSTANCE = new JzAzBz();

    private static final String[] NAMES = {"Jz", "az", "bz"};
    private static final double[] MINS = {0.0, -0.15, -0.20};
    private static final double[] MAXS = {0.25, 0.15, 0.15};
    ...
    /**
     * Computes the inverse of a 3x3 matrix via cofactor expansion.
     *
     * @param m 3x3 matrix
    ...
    public double[] toParent(double[] raw) {
        ...
        // Remove SDR scaling
        return new double[] {X / SDR_WHITE, Y / SDR_WHITE, xyzP[2] / SDR_WHITE};
    }

    @Override
    public double[] fromParent(double[] parentRaw) {
        // Scale from library XYZ [0,1] to absolute nits (BT.2408)
        double X = parentRaw[0] * SDR_WHITE;
        double Y = parentRaw[1] * SDR_WHITE;
        double Z = parentRaw[2] * SDR_WHITE;
